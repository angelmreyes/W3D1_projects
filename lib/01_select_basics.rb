...
def scandinavia
  ...
  FROM
    countries
  WHERE
    countries.name = 'Denmark' OR countries.name = 'Finland' 
    OR countries.name = 'Norway' OR countries.name = 'Sweden'; 
  SQL
end
...
def starts_with_g
  ...
  WHERE 
    name LIKE 'G%'; 
  SQL
   #you can find (countries.name LIKE 'G%' or name LIKE 'G%')
   #countries.name is useful and necessary when joining two tables so that we can
   #specify from which table did the column name come from. 
end
